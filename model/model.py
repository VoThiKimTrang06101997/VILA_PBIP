...
# from model.model_ViLa_MIL import PromptLearner, TextEncoder
# from model.segform import mix_transformer
# from model.model_utils import Attn_Net_Gated, attention_diversity
...
# ROOT_FOLDER = r"E:\NghienCuu_WSSS\ProtoBIP"
...
#         self,
#         backbone='mit_b1',
#         cls_num_classes=5,  # Default to 5 classes (4 foreground + 1 background)
#         stride=[4, 2, 2, 1],
#         pretrained=True,
#         n_ratio=0.5,
#         l_fea_path=None,
...
#     ):
#         super().__init__()
#         self.cls_num_classes = cls_num_classes  # 5 classes including background
#         self.total_classes = cls_num_classes    # Already includes background
#         self.stride = stride
...
#         self.l_fea = None
#         self.k_list = k_list if k_list is not None else [1, 1, 1, 1, 1]  # Default to 5 classes
#         if len(self.k_list) != cls_num_classes:
#             logger.warning(f"k_list length {len(self.k_list)} does not match cls_num_classes {cls_num_classes}. Adjusting k_list to {cls_num_classes} elements.")
#             self.k_list = [1] * cls_num_classes
#         if sum(self.k_list) != cls_num_classes:  # Ensure sum matches total classes
#             logger.warning(f"k_list sum {sum(self.k_list)} does not match cls_num_classes {cls_num_classes}. Adjusting to equal distribution.")
#             self.k_list = [1] * cls_num_classes  # Force equal distribution for all 5 classes
...
#             "A WSI of Background with no specific tissue structure."
#         ]
...
#         # Class weights for imbalanced data, boosting LYM and Background
#         self.class_weights = torch.tensor([1.0, 1.0, 2.5, 2.0, 2.0], device=self.device)  # Higher weight for LYM (2.5) and Background (2.0)
...
#             if cam.size(1) != self.total_classes:
#                 logger.warning(f"CAM has {cam.size(1)} channels, expected {self.total_classes}. Adjusting.")
#                 if cam.size(1) < self.total_classes:
#                     padding = torch.zeros(cam.size(0), self.total_classes - cam.size(1), cam.size(2), cam.size(3), device=cam.device)
#                     cam = torch.cat([cam, padding], dim=1)
#                 elif cam.size(1) > self.total_classes:
#                     cam = cam[:, :self.total_classes]
#             return cam
...
#         if cam1.size(1) != self.total_classes or cam2.size(1) != self.total_classes or cam3.size(1) != self.total_classes or cam4.size(1) != self.total_classes:
#             logger.error(f"Invalid CAM channels: cam1={cam1.size(1)}, cam2={cam2.size(1)}, cam3={cam3.size(1)}, cam4={cam4.size(1)}. Expected {self.total_classes}.")
#             return [torch.zeros(1, device=device)] * 13

#         # Use labels as cls_label if provided, otherwise handle gracefully
#         cls_label = labels if labels is not None else torch.zeros(batch_size, self.total_classes, device=device)
#         cls_label_with_bg = cls_label  # Already includes background

#         # Apply class weights to loss
#         if cls_label_with_bg is not None:
#             weights = self.class_weights.repeat(batch_size, 1)
#             loss_function = nn.BCEWithLogitsLoss(weight=weights)
#         else:
#             loss_function = self.loss_bce

#         batch_info1 = self.feature_extractor.process_batch(x, cam1, cls_label) if self.l_fea is not None and cls_label is not None else None
#         batch_info2 = self.feature_extractor.process_batch(x, cam2, cls_label) if self.l_fea is not None and cls_label is not None else None
#         batch_info3 = self.feature_extractor.process_batch(x, cam3, cls_label) if self.l_fea is not None and cls_label is not None else None
#         batch_info4 = self.feature_extractor.process_batch(x, cam4, cls_label) if self.l_fea is not None and cls_label is not None else None
...
#         cls1_merge = merge_to_parent_predictions(cls1, self.k_list, method=merge_method, include_background=True)
#         cls2_merge = merge_to_parent_predictions(cls2, self.k_list, method=merge_method, include_background=True)
#         cls3_merge = merge_to_parent_predictions(cls3, self.k_list, method=merge_method, include_background=True)
#         cls4_merge = merge_to_parent_predictions(cls4, self.k_list, method=merge_method, include_background=True)
...
#         loss = loss_function(cls4_merge, cls_label_with_bg) + 0.25 * diversity_loss if cls_label_with_bg is not None and self.l_fea is not None else torch.tensor(0.0, device=device)

#         k_list = self.k_list  # Ensure k_list is consistently [1, 1, 1, 1, 1]
...
class ClsNetwork(nn.Module):
    def __init__(
        ...
        self.l_fea = None
        self.k_list = k_list if k_list is not None else [1, 1, 1, 1]
