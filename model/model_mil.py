...
class MIL_fc(nn.Module):
    def __init__(self, gate=True, size_arg="small", dropout=False, n_classes=4, top_k=1):
        super(MIL_fc, self).__init__()
        self.size_dict = {"small": [512, 512]}
    ...
    def forward(self, h, return_features=False):
        if h.dim() != 2 or h.shape[1] != 512:
            raise ValueError(f"Expected input shape [batch_size, 512], got {h.shape}")
